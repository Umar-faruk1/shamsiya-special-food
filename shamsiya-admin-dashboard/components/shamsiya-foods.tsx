...
function FoodForm({
  ...
  const [values, setValues] = useState<FoodFormValues>(
    food
      ? {
          ...
          preparation_time: String(food.preparation_time ?? 15),
  ...
  const set = <K extends keyof FoodFormValues>(
    key: K,
  ...
  async function save() {
    ...
    try {
      const ingredientsValue = values.ingredients
        .split(",")
      ...
      setError(error instanceof Error ? error.message : "Unable to save food.");
    } finally {
      setSaving(false);
    }
  }
      ...
      >
        <div className="drawer-heading">
          <div>
            ...
            <h2 id="food-form-title">{food ? "Edit food" : "Add new food"}</h2>
          </div>
          <button
        ...
        <div className="food-form-grid">
            ...
            <div className="two-fields">
              ...
              <label className="field-label">
                ...
                <input
                  ...
                  onChange={(event) =>
                    set("preparation_time", event.target.value)
                  }
                />
              </label>
            </div>
          ...
          <div>
            <div className="field-label">
              ...
              <label className="image-upload-area" htmlFor="food-image">
                {imagePreview ? (
                  <img
                    src={imagePreview}
                    alt="Food preview"
...
export function FoodsContent() {
  ...
  async function load() {
    ...
    try {
      ...
      setCategories(
        (categoryData as Category[]).filter((item) => item.is_active) ?? [],
      );
    } catch (error) {
      setError(
  ...
  const filtered = useMemo(
    ...
    [foods, query, category, availability],
  );
  const closeForm = () => {
    setFormOpen(false);
    setSelected(null);
  };
  const averageRating = foods.length
    ? foods.reduce((sum, food) => sum + (food.rating ?? 0), 0) / foods.length
    : 0;
    ...
    <main className="main-content foods-main">
      <div className="page-heading overview-heading">
        <div>
          ...
          <h1>Foods</h1>
          <p>Manage your menu, pricing, availability, and featured items.</p>
        </div>
      ...
      <section className="stats-grid food-stats">
        ...
        <article className="stat-card">
          ...
          <div className="stat-value">
            {foods.filter((food) => food.available).length}
          </div>
          <div className="stat-foot">
            <span>Live</span>
            <span>for ordering</span>
          </div>
        </article>
        <article className="stat-card">
          <div className="stat-head">
          ...
          <div className="stat-value">
            {foods.filter((food) => food.featured).length}
          </div>
          <div className="stat-foot">
            <span>Live</span>
                  ...
                  <tr key={food.id}>
                    ...
                    <td>
                      <span className="category-chip">
                        {food.categories?.name ?? "Uncategorized"}
                      </span>
                    </td>
                    <td>
                      <strong>
                    ...
                    <td>
                      <span className="food-rating">
                        <Star /> {food.rating?.toFixed(1) ?? "—"}
                      </span>
                    </td>
                    <td>
                      <span
                        className={`status-badge ${food.available ? "status-success" : "status-pending"}`}
                      >
                        <span />
